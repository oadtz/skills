...
Go line by line and ask "so what?" of each claim. If a sentence survives "so what?" three times, it
earns its place. If it dead-ends, cut it or make it concrete. This strips out the abstract filler
that deflates most pitches.

## Common message-deflating mistakes

- **Leading with yourself.** Opening on "we are a company that…" instead of the audience's world or
  pain. Move the spotlight off you.
- **Feature soup.** A list of capabilities with no story to hang them on. Reframe features as gifts
...
Go back up the pipeline, not deeper into design. A flat pitch usually means a weak core message
(Step 2) or the wrong narrative arc for the audience (Step 3) — or, sometimes, an idea that isn't
actually compelling yet, which is a signal to return to `ideakit-validate` rather than to polish.
